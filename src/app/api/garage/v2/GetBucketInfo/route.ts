...
export async function GET(request: NextRequest) {
  ...
  const globalAlias = searchParams.get('globalAlias');
  const search = searchParams.get('search');

  // 构建目标URL的查询参数
  const targetParams = new URLSearchParams();
  if (id) targetParams.set('id', id);
  if (globalAlias) targetParams.set('globalAlias', globalAlias);
  if (search) targetParams.set('search', search);

  const queryString = targetParams.toString();
  const targetUrl = `${GARAGE_API_BASE_URL}/v2/GetBucketInfo${queryString ? `?${queryString}` : ''}`;
