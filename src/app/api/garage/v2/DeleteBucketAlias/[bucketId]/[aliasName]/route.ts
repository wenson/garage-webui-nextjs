...
export async function POST(
  request: NextRequest,
  { params }: { params: Promise<{ bucketId: string; aliasName: string }> }
) {
  const { bucketId, aliasName } = await params;

  const response = await fetch(
    `${GARAGE_API_BASE_URL}/v2/DeleteBucketAlias?bucketId=${bucketId}&aliasName=${aliasName}`,
    {
      method: 'POST',
      headers: {
