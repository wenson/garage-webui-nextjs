...
export async function GET(request: NextRequest) {
  ...
  const showSecretKey = searchParams.get('showSecretKey');

  // 构建目标URL的查询参数
  const targetParams = new URLSearchParams();
  if (id) targetParams.set('id', id);
  if (search) targetParams.set('search', search);
  if (showSecretKey) targetParams.set('showSecretKey', showSecretKey);

  const queryString = targetParams.toString();
  const targetUrl = `${GARAGE_API_BASE_URL}/v2/GetKeyInfo${queryString ? `?${queryString}` : ''}`;
